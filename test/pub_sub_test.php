...
class PubSubTest extends PHPUnit_Framework_TestCase {
  ...
  public function testGet() {
    ...
    $this->assertEmpty(PubSub::events('my_event', 50));

    // same priority, different event
    $this->assertEmpty(PubSub::events('some_other_event', 100));

    // diffent event, no priority distinction
    $this->assertEmpty(PubSub::events('some_other_event'));

    $callbacks = PubSub::events('my_event');
  ...
  public function testRemove() {
    ...
    foreach ($callbacks as $key => $callback) {
      PubSub::subscribe('my_event', $callback);
      $this->assertEquals(1, count(PubSub::events('my_event')), '1 callback was added, so 1 callback should be in array.');
      PubSub::unsubscribe('my_event', $callback);
      $this->assertEquals(0, count(PubSub::events('my_event')), '1 callback was unsubscribed, so 0 callbacks should be in array.');
    }
  }
